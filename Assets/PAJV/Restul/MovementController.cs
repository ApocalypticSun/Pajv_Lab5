...
[RequireComponent(typeof(Rigidbody))]
    public class MovementController : MonoBehaviour {
        ...
        public float CurrentSpeed { get { return _rigidBody.linearVelocity.magnitude; } }
        ...
        protected virtual void Move(){
            ...
            if(_rigidBody.linearVelocity.magnitude <= _stopSlideSpeed && _movementControls.MoveDirection == Vector2.zero)
                _rigidBody.linearVelocity = Vector3.zero;
            float targetAngle = cameraTransform.eulerAngles.y;
            Quaternion targetRotation = Quaternion.Euler(0, targetAngle, 0);
            Quaternion smoothedRotation = Quaternion.RotateTowards(_rigidBody.rotation, targetRotation, 360f * Time.fixedDeltaTime);
            _rigidBody.MoveRotation(smoothedRotation);
            // adjusts for max speeds
            float speedFactor = (_currentMaxSpeed - _rigidBody.linearVelocity.magnitude ) / _currentMaxSpeed;
            moveDirection = new Vector3(_movementControls.MoveDirection.x, 0f, _movementControls.MoveDirection.y);
            moveDirection = Quaternion.AngleAxis(targetAngle, Vector3.up) * moveDirection;
            newDir = moveDirection;
